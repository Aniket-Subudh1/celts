...
export function StudentScore() {
  const [stats, setStats] = useState<StudentStatsDoc | null>(null);
  const [tests, setTests] = useState<TestSubmissionGroup[]>([]);
  ...
  async function fetchSubmissions() {
    const res = await api.apiGet("/student/submissions");
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="flex justify-between items-start">
          <div>
            ...
            {stats.batchName && (
              <p className="text-xs text-slate-500 mt-1">
                Batch:{" "}
                <span className="text-slate-700">{stats.batchName}</span>
              </p>
            )}
            ...
            <div className="mt-2 text-sm text-slate-500">
              ...
              {stats.systemId && (
                <span className="ml-4">
                  ID:{" "}
                  <span className="text-slate-700">{stats.systemId}</span>
                </span>
              )}
            </div>
          </div>
      ...
      {/* ================= TEST RESULTS ================= */}
      <Card className="p-6 rounded-xl">
        <h3 className="text-lg font-semibold mb-4">My Test Results</h3>
        ...
        <div className="space-y-3">
          {tests.map((test) => {
            const open = expandedTestId === test.testId;
            ...
            return (
              ...
              >
                <button
                  onClick={() =>
                    setExpandedTestId(open ? null : test.testId)
                  }
                ...
                {open && (
                  <div className="p-4 border-t space-y-4 bg-slate-50">
                    {test.submissions.map((s) => (
                      <div key={s.submissionId} className="text-sm">
                        <p className="font-medium capitalize">
                          {s.skill} — Band{" "}
                          <span className="font-semibold">
                            {formatBand(s.bandScore)}
                          </span>
                        </p>
                        ...
                        {(s.skill === "reading" ||
                          ...
                          <div className="grid grid-cols-2 md:grid-cols-4 gap-2 mt-2">
                            <div>Total: {s.totalQuestions}</div>
                            <div>Attempted: {s.attemptedCount}</div>
                            <div className="text-emerald-700">
                        ...
                        {(s.skill === "writing" ||
                          s.skill === "speaking") &&
                          s.examinerSummary && (
                            <div className="mt-3 p-3 bg-white border rounded whitespace-pre-wrap text-sm">
                              {s.examinerSummary}
                            </div>
                          )}
                      </div>
